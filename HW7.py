...
def make_players_table(data, cur, conn):
    ...
    # Iterate through the players in the squad
    for player in data['squad']:
        player_id = player['id']
        name = player['name']
        ...
        # Look up the position_id for the player's position
        cur.execute("SELECT Positions.id FROM Positions WHERE position = ?", [player_position])
        row = cur.fetchone()
        if row:
            position_id = row[0]
        else:
        ...
        # Calculate the player's birthyear
        birthyear = int(birthdate[:4])

        # Insert the player's information into the Players table
...
def nationality_search(countries, cur, conn):
    ...
    # Create the query using the placeholders
    query = f"""SELECT name, position_id, nationality FROM players WHERE nationality IN ({placeholders})"""
    
    # Execute the query with the countries as parameters
    cur.execute(query, countries)
    
    # Fetch all the results and return as a list of tuples
...
def make_winners_table(data, cur, conn):
    ...
    for season in data['seasons']:
        ...
        if winner:
            cur.execute('INSERT OR IGNORE INTO Winners (id, name) VALUES (?, ?)', (winner['id'], winner['name']))
    conn.commit()
...
def winners_since_search(year, cur, conn):
    cur.execute('SELECT name, end_year FROM Seasons JOIN Winners ON Seasons.winner_id = Winners.id WHERE end_year >= ?', (year,))
    rows = cur.fetchall()
    winners_dict = {}
    for row in rows:
        winner_name = row[0]
        end_year = row[1]
        if winner_name in winners_dict:
            winners_dict[winner_name] += 1
        else:
            winners_dict[winner_name] = 1
    conn.commit()
    return winners_dict
